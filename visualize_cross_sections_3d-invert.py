...
z_separation = 0.2  # Separation between sections on the Z-axis
...
white_threshold = 200  # Threshold for pixels to be considered white (after inversion)
...
glClearColor(0.82, 0.651, 0.392, 1.0)  # Set background to brown (RGB: 139, 69, 19)
glDisable(GL_CULL_FACE)  # Disable back-face culling to render both sides of quads
glEnable(GL_DEPTH_TEST)  # Enable depth testing

# Load textures (seccion_1_string_art.png to seccion_98_string_art.png) with inverted colors
textures = []
for i in range(1, num_sections + 1):
    ...
    if os.path.exists(img_path):
        try:
            ...
            # Invert colors (255 - value) for RGB channels
            img_data = 255 - img_data
            # Binarize: pixels with any RGB >= white_threshold are white (255, 255, 255), others transparent
            ...
            # Set non-white pixels to black (will be transparent)
            img_data[~white_mask] = [0, 0, 0]
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST)  # Use NEAREST to avoid interpolation
